    ...
    methods: {
      ...
      async clickTaskDoing(item , tabname = 1){
        if(item.tname == 'bs_seal_regist' && item.topic.includes('#待用印')){
          ...
          this.$router.push(`/app/goodsview?id=${item.id}&statustype=${workconfig.goodstype[item.type]}&type=${workconfig.goodstype[item.type]}&role=front&view=edit&back=/explore`); //跳转到相应的用印界面
        } else if(item.tname == 'bs_goods_receive' && item.topic.includes('#已领取')){
          this.$router.push(`/app/goodsview?id=${item.id}&statustype=${workconfig.goodstype[item.type]}&type=${workconfig.goodstype[item.type]}&role=receive&view=edit&back=/explore`); //跳转到相应的用印界面
        } else {
