      ...
      <div class="wechat-list">
        ...
        <template v-show="tabname == 1 && !loading && !isLoading">
          ...
          <div class="list-info" style="margin-top:0px;" v-show="tabname == 1 && !loading && !isLoading" :key="item.id" v-for=" (item , index) in timetasks.slice(0,8)">
              ...
              <div class="desc-box" @click="$router.push(`/explore/content?id=${item.id}&pid=${item.pid}&tasktype=wait&tname=${item.tname}&bname=${encodeURI(item.name)}&username=${item.proponents}&sponsor=${encodeURI(item.sponsor)}&topic=${encodeURI(item.topic)}&title=${encodeURI(item.topic)}&files=${encodeURI(item.files)}`)">
         ...
         <template v-show="tabname == 3 && !loading && !isLoading">
          ...
          <div class="list-info" v-show="tabname == 3 && !loading && !isLoading" :key="item.id" v-for=" (item , index) in donetasks.slice(0,8)">
            ...
            <div class="desc-box" @click="$router.push(`/explore/content?id=${item.id}&pid=${item.pid}&tasktype=done&tname=${item.tname}&bname=${encodeURI(item.name)}&username=${item.proponents}&sponsor=${encodeURI(item.sponsor)}&topic=${encodeURI(item.topic)}&title=${encodeURI(item.topic)}&files=${encodeURI(item.files)}`)">
              <div class="desc-time">{{item.create_time}}</div>
              <div class="desc-author">{{`${item.type} - ${item.name}`}}</div>
              <div class="desc-msg">
                <div class="desc-mute iconfont icon-mute" style="display: none;"></div>
         ...
         <template v-show="tabname == 4 && !loading && !isLoading">
          ...
          <div class="list-info" v-show="tabname == 4 && !loading && !isLoading" :key="item.id" v-for=" (item , index) in announces.slice(0,8)">
              ...
              <div class="desc-box" @click="$router.push(`/explore/announce?id=${item.id}&pid=${item.pid}&type=${item.announce_type}&title=${encodeURI(item.title)}&content=${encodeURI(item.content)}&files=${encodeURI(item.files)}&tabname=4&from=/explore`)">
                <div class="desc-time">{{item.create_time}}</div>
