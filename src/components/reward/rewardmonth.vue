              ...
              <div class="reward-apply-content" style="height:auto; background-color:#fefefe; margin-top:0px; margin-left: 5rem; margin-right: 5rem; margin-bottom: 5rem; border: 1px solid #f0f0f0; front-size: 1rem;" >
                ...
                <div class="reward-apply-content-item" style="margin-top:5px;margin-bottom:5px;">
                  <a-row>
                    ...
                    <a-col :span="8">
                       <a-input readonly v-model="item.month" placeholder="请输入报表月份！" style="border: 0px solid #fefefe;  border-bottom: 1px solid #f0f0f0;" />
                    </a-col>
                  </a-row>
                </div>
    ...
    //上传文件成功后回调函数
      ...
      async queryInfo() {
        ...
        try {
          ...
          this.bpm_status = tools.getUrlParam('bpm_status'); //获取bpm状态
          this.proponents = tools.getUrlParam('proponents'); //获取此申请的流程应发起人
      ...
      async handleDisagree(){
        ...
        await manageAPI.patchTableData(this.tablename , id , { id,  status: '待审批', bpm_status: '100', });
      ...
      async handleNotifyHR(user_group_ids , userinfo ,  value , receiveURL){
        try {
          await superagent.get(`${window.requestAPIConfig.restapi}/api/v1/weappms/${user_group_ids}/亲爱的同事，${userinfo.realname}(${userinfo.username})提交了奖罚申请流程，请您在流程审批完成后及时进行知会确认处理！?rurl=${receiveURL}`)
                          .set('accept', 'json');
        } catch (error) {
          console.log(error);
        }
