          ...
          <van-cell-group>
            <van-form >
              ...
              <van-cell-group style="margin-top:10px;">
                ...
                <van-address-list v-show="hContractList.length > 0 && item.sealtype == '合同类' && item.status != '已退回' && item.status != '已作废' " v-model="hContractID" :list="hContractList" default-tag-text="默认" edit-disabled @select="selectHContract()" />
              ...
              <van-cell-group style="margin-top:10px;">
                ...
                <van-field v-show="item.sealtype == '合同类' && false " required clearable label="前台客服" v-model="item.front_name" placeholder="请输入前台客服人员名称" @blur="validField('front');queryFrontMan();" :error-message="message.front" @click="queryFrontMan();" />
      ...
      async queryInfo(){
        ...
        try {
          ...
          if(that.item.sealman){
            //获取可选填报人列表
            let slist = await Betools.manage.queryUsernameByIDs(that.item.seal.split(',').map(item => { return `'${item}'`; }).join(','));
            //遍历填报人列表
