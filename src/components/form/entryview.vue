...
export default {
    ...
    methods: {
              ...
              if(Array.isArray(user)){
                ...
                try {
                  user.map((elem,index) => {
                    ...
                    this.huserList.push({id:elem.loginid , value:`${user.lastname},` , label: elem.lastname + ' ' +  elem.mobile + " " + elem.textfield1.split('||')[1].replace('中心','') , name:elem.lastname , tel:'' , address: company + "||" + elem.textfield1.split('||')[1] , company: company , department:department , mail: elem.email , isDefault: !index });
                  })
                } catch (error) {
                  console.log(error);
                }
              ...
              if(Array.isArray(user)){
                ...
                try {
                  user.map((elem,index) => {
                    ...
                    let department = elem.textfield1.split('||')[1];
                    department = department.slice(department.lastIndexOf('>')+1);
                    this.fuserList.push({id:elem.loginid , value:`${user.lastname},` , label: elem.lastname + ' ' +  elem.mobile + " " + elem.textfield1.split('||')[1].replace('中心','') , name:elem.lastname , tel:'' , address: company + "||" + elem.textfield1.split('||')[1] , company: company , department:department , mail: elem.email , isDefault: !index });
                  })
                } catch (error) {
                  console.log(error);
                }
              ...
              if(Array.isArray(user)){
                ...
                try {
                  user.map((elem,index) => {
                    ...
                    let department = elem.textfield1.split('||')[1];
                    department = department.slice(department.lastIndexOf('>')+1);
                    this.muserList.push({id:elem.loginid , value:`${user.lastname},` , label: elem.lastname + ' ' +  elem.mobile + " " + elem.textfield1.split('||')[1].replace('中心','') , name:elem.lastname , tel:'' , address: company + "||" + elem.textfield1.split('||')[1] , company: company , department:department , mail: elem.email , isDefault: !index });
                  })
                } catch (error) {
                  console.log(error);
                }
      ...
      async queryInfo() {
        ...
        try {
          ...
          if(!(this.role == 'hr' || this.role == 'done')){
            ...
            value.diploma = value.diploma ? value.diploma.slice(0,4) + '********' + value.diploma.slice(12 - value.diploma.length) : '';
            value.bachelor = value.bachelor ? value.bachelor.slice(0,4) + '********' + value.bachelor.slice(12 - value.bachelor.length) : '';

            value.diplomass = value.diplomass ? value.diplomass.slice(0,4) + '********' + value.diplomass.slice(12 - value.diplomass.length) : '';
            value.bachelorss = value.bachelorss ? value.bachelorss.slice(0,4) + '********' + value.bachelorss.slice(12 - value.bachelorss.length) : '';

            value.diplomabs = value.diplomabs ? value.diplomabs.slice(0,4) + '********' + value.diplomabs.slice(12 - value.diplomabs.length) : '';
            value.bachelorbs = value.bachelorbs ? value.bachelorbs.slice(0,4) + '********' + value.bachelorbs.slice(12 - value.bachelorbs.length) : '';
          }
      ...
      async queryUserInfo(name , queryURL = '', resp = '', front = {id:''}){
        // 查询前台人员SQL
        queryURL = `${window.requestAPIConfig.restapi}/api/bs_hrmresource?_where=((lastname,like,~${name}~)~or(loginid,like,~${name}~))~and(status,ne,5)&_fields=id,lastname,loginid,textfield1,sex`;
      ...
      async handleConfirm() {
        ...
        // 返回预览URL
        const receiveURL = encodeURIComponent(`${window.location.protocol}//${window.location.host}/#/app/entryview?id=${id}&statustype=none&role=`);
      ...
      async handleFinaly(){
        ...
        if(!Betools.tools.isNull(value.front_time) && !Betools.tools.isNull(value.admin_time) && !Betools.tools.isNull(value.meal_time)){
           ...
           // 返回预览URL
          const receiveURL = encodeURIComponent(`${window.location.protocol}//${window.location.host}/#/app/entryview?id=${id}&statustype=none&role=done`);
