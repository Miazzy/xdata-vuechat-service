...
export default {
    ...
    methods: {
        ...
        async queryTabList(tabname, page) {
            ...
            this.initContractList = await Betools.manage.queryTableData('bs_contract_transfer_apply', `_where=(status,eq,${status})~and(create_time,gt,${month})~and(type,like,~档案移交~)~and(receive_ids,in,${userinfo.username})${sealTypeSql}${searchSql}&_sort=-create_time&_p=0&_size=1000`);
