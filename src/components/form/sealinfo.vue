...
export default {
    ...
    methods: {
        ...
        async companySearch(data, key) {
            ...
            const companyAPI = `https://api.yunwisdom.club:30443/gateway-config/system.admin.config`;
            ...
            if((Betools.tools.isNull(data) || data.length == 0)){
                //查询数据库的公司列表信息
                data = await Betools.manage.queryTableData('bs_company_flow_base', `_where=(status,in,0)~and(level,gt,2)~and(name,like,~${key}~)&_fields=id,ename,name&_sort=id&_p=0&_size=30`); // 获取最近12个月的已用印记录
                data = data.map((item, index) => { item.title = item.code = item.name; });
