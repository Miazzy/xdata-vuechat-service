    ...
    methods: {
      ...
      async queryInfo() {
        ...
        try {
          ...
          //根据URL参数查询物资类型
          this.item.type = this.goodsborrowtype[Betools.tools.getUrlParam('type')];
      ...
      async handleApply() {
        ...
        // 返回预览URL
        const receiveURL = encodeURIComponent(`${window.location.protocol}//${window.location.host}/#/app/borrowview?id=${id}&statustype=office&type=${type}&role=front`);
        ...
        await superagent.get(`${window.requestAPIConfig.restapi}/api/v1/weappms/${user_group_ids}/物品借用登记通知：员工‘${userinfo.realname}(${userinfo.username})’ 部门:‘${userinfo.department.name}’ 单位:‘${userinfo.parent_company.name}’ 序号:‘${value.serialid}’ 物品借用登记完毕，请确认！?rurl=${receiveURL}`)
                .set('accept', 'json');
