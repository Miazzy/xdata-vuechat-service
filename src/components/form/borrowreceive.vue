          ...
          <van-cell-group>
            ...
            <van-form >
              ...
              <van-cell-group style="margin-top:10px;position:relative;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly" required clearable label="数量/单位" v-model="item.amount"  placeholder="请填写借用数量及单位，如2支！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=2" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount1"  placeholder="请填写借用数量及单位，如2支！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=3" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount2"  placeholder="请填写借用数量及单位，如1盒！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=4" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount3"  placeholder="请填写借用数量及单位，如4个！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=5" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount4"  placeholder="请填写借用数量及单位，如2支！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=6" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount5"  placeholder="请填写借用数量及单位，如2盒！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=7" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount6"  placeholder="请填写借用数量及单位，如2只！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=8" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount7"  placeholder="请填写借用数量及单位，如2块！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=9" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount8"  placeholder="请填写借用数量及单位，如1袋！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=10" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount9"  placeholder="请填写借用数量及单位，如2个！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=11" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount10"  placeholder="请填写借用数量及单位，如2块！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=12" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount11"  placeholder="请填写借用数量及单位，如2箱！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=13" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount12"  placeholder="请填写借用数量及单位，如2个！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=14" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount13"  placeholder="请填写借用数量及单位，如2只！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group v-show="size>=15" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount14"  placeholder="请填写借用数量及单位，如2盒！" @blur="validField('amount')" :error-message="message.amount"  />
               ...
               <van-cell-group v-show="size>=16" style="margin-top:10px;position:relative;border-top:0px solid #fefefe;">
                ...
                <!-- 借用数量（HR需要确认/修改） -->
                <van-field :readonly="readonly"  clearable label="数量/单位" v-model="item.amount15"  placeholder="请填写借用数量及单位，如2箱！" @blur="validField('amount')" :error-message="message.amount"  />
              ...
              <van-cell-group id="van-user-list" class="van-user-list" style="margin-top:10px;">
                ...
                <van-field required clearable label="物品管理员" v-model="item.user_admin_name" placeholder="请输入物品借用管理员!" @blur="validField('user_admin_name');querySealMan();" :error-message="message.user_admin_name" @click="querySealMan();" />
...
export default {
    ...
    methods: {
            ...
            if(!!user){
              ...
              if(Array.isArray(user)){
                ...
                try {
                  ...
                  department = department.slice(department.lastIndexOf('>')+1);
                  //将用户数据推送至对方数组
                  this.userList.push({id:user.loginid , name:user.lastname , tel:'' , address: company + "||" + user.textfield1.split('||')[1] , company: company , department:department , mail: this.item.dealMail, isDefault: !this.suserList.length });
                  //获取盖印人姓名
      ...
      async selectSealUser(value){
        ...
        this.item.user_admin_name = user.name;  //获取盖印人姓名
      },
      ...
      async queryInfo() {
        try {
          ...
          const item = Betools.storage.getStore(`system_${this.tablename}_item#${this.item.type}#@${userinfo.realname}`);  //获取缓存信息
          this.item.type = this.goodsborrowtype[Betools.tools.getUrlParam('type')];  //根据URL参数查询物资类型
          //自动回显刚才填写的用户基础信息
          if(item){
      ...
      async handleApply() {
        ...
        if(Betools.tools.isNull(this.item.userid)){
          ...
          await vant.Dialog.alert({
              title: '温馨提示',
              message: '请输入物品管理员并点击人员列表，选择物品借用管理员！',
            });
          return;
        }
        ...
        // 返回预览URL
        const receiveURL = encodeURIComponent(`${window.BECONFIG.domain.replace('www','wechat')}/#/app/borrowview?id=${id}&statustype=office&type=${type}&role=front`);
        ...
        await superagent.get(`${window.BECONFIG['restAPI']}/api/v1/weappms/${user_group_ids}/物品借用登记通知：员工‘${userinfo.realname}(${userinfo.username})’ 部门:‘${userinfo.department.name}’ 单位:‘${userinfo.parent_company.name}’ 序号:‘${value.serialid}’ 物品借用登记完毕，请确认！?rurl=${receiveURL}`)
