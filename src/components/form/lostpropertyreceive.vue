...
export default {
    ...
    data() {
        return {
            ...
            message: Betools.workconfig.compValidation.lostproperty.message,
            valid: Betools.workconfig.compValidation.lostproperty.valid,
            item:{
              id: '',
              serialid:'',
    ...
    methods: {
      ...
      async handleApply() {
        ...
        // 返回预览URL
        const receiveURL = encodeURIComponent(`${window.location.protocol}//${window.location.host}/#/app/lostpropertyview?id=${id}&statustype=office&type=${type}&role=front`);
        ...
        await superagent.get(`${window.BECONFIG['restAPI']}/api/v1/weappms/${user_group_ids}/失物招领登记通知：员工‘${userinfo.realname}(${userinfo.username})’ 部门:‘${userinfo.department.name}’ 单位:‘${userinfo.parent_company.name}’ 序号:‘${value.serialid}’ 失物招领登记完毕！?rurl=${receiveURL}`)
                .set('accept', 'json');
