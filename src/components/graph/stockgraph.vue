  ...
  methods: {
    setGraphData() {
      ...
      setTimeout(function() {
        ...
        this.$refs.seeksRelationGraph.setJsonData(__graph_json_data, (seeksRGGraph) => {  // 这些写上当图谱初始化完成后需要执行的代码
        });
      }.bind(this), 1000)
    },
    onNodeExpand(node, e) {
      ...
      if (node.data && node.data.asyncChild === true && node.data.loaded === false) {
        ...
        setTimeout(function() {
          ...
          this.$refs.seeksRelationGraph.appendJsonData(_new_json_data, (seeksRGGraph) => { // 这些写上当图谱初始化完成后需要执行的代码
          });
        }.bind(this), 1000)
        return
      }
