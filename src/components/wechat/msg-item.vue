<template>
    ...
    <li :class="{'item-hide':deleteMsg}">
        ...
        <router-link :to="{ path: '/wechat/dialogue', query: { wxid:item.heuserid, mid: item.mid,name:item.group_name||(item.user[0].remark||item.user[0].nickname),group_num:item.user.length}}" tag="div" class="list-info" v-swiper v-on:click.native="toggleMsgRead($event,'enter')">
            <div class="header-box">
                ...
                <div class="header" :class="[item.type=='group'?'multi-header':'']">
                    <img :key="userInfo.avatar" v-for="userInfo in item.user" :src="userInfo.avatar">
                </div>
            </div>
            <div class="desc-box">
                <!--使用过滤器 fmtDate 格式化时间-->
