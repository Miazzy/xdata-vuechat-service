...
export async function queryUserByNameHRM(name) {
    ...
    try {
        //如果用印登记类型为合同类，则查询最大印章编号，然后按序使用更大的印章编号
        var maxinfo = await superagent.get(`${window.requestAPIConfig.restapi}/api/v1/hrmresource/id?_where=(lastname,like,%27~${name}~%27)~and(status,ne,5)`).set('accept', 'json');
        //返回用户信息
...
export async function queryContractInfoByPrefix(prefix) {
    ...
    try {
        //构建查询SQL
        const sql = `${window.requestAPIConfig.restapi}/api/bs_seal_regist?_where=(prefix,like,${prefix}~)&_p=0&_size=3&_sort=-create_time`;
        //如果用印登记类型为合同类，则查询最大印章编号，然后按序使用更大的印章编号
